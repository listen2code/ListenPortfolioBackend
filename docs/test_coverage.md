...
- **认证测试**：登录、注册、Token 验证
- **授权测试**：权限控制和访问限制
- **限流测试**：各种限流场景验证与时间窗口临界点隔离保护
- **输入验证**：参数校验和 SQL 注入防护

> [!TIP]
> **限流测试并发/跨窗口 Flaky 防护设计**：
> 1. **UUID Key 隔离**：测试用例（如 `SecurityTest`）采用 `UUID.randomUUID()` 动态生成限流 Key（如 `security:test:${UUID}`），彻底避免重复构建或多个用例间继承历史 Redis 限流 Key 造成的污染。
> 2. **时间窗口跨界避让缓冲**：在测试高频限流拦截（如 20 次密集请求）时，防范 `System.currentTimeMillis() / (timeWindow * 1000)` 恰好在循环中途切入下一个 10s/60s 窗口导致计数器重置。测试代码会自动判定时间戳是否处于窗口即将切换的最后 2 秒内，并自动执行毫秒级避让，保证 100% 在单一时间窗口内完成断言。
